...
file_array = ["../../MSR_data/rsrch_0.csv.gz", "../../MSR_data/rsrch_1.csv.gz", "../../MSR_data/rsrch_2.csv.gz"]
...
class_name = "home"
f = open("rsrch_full_MSR_30min.csv", "w+")
...
for index, row in df.iterrows():
    ...
    if (time_diff.total_seconds() > 1800):
        ...
        read_size_mean = total_read_size/num_read if (num_read) else 0
        write_size_mean = total_write_size/num_write if (num_write) else 0
        ...
        total_avg_seq = total_seq_length/total_seq if (total_seq) else 0
        ...
        read_avg_seq = total_seq_length/total_seq if (total_seq) else 0
        ...
        for seq in write_all_seq:
            total_seq += len(seq)
            total_seq_length += np.sum(seq)
        write_avg_seq = total_seq_length/total_seq if (total_seq) else 0

        # AVERAGE ACCESS DISTANCE
        avg_read_access = np.mean(read_access_distance) if (len(read_access_distance)) else 0
        avg_write_access = np.mean(write_access_distance) if (len(write_access_distance)) else 0
        avg_total_access = np.mean(total_access_distance) if (len(total_access_distance)) else 0

        #BURSTINESS
        burst_read = np.var(read_timestamp_array)/np.mean(read_timestamp_array) if (len(read_timestamp_array)) else 0
        burst_write = np.var(write_timestamp_array)/np.mean(write_timestamp_array) if (len(write_timestamp_array)) else 0
        burst_total = np.var(total_timestamp_array)/np.mean(total_timestamp_array) if (len(total_timestamp_array)) else 0
