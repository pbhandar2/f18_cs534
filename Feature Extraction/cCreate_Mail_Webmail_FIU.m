...
mail_folder{1} = dir( 'FIU/mail/webmail' ) ;
...
for i_folder = 1 : numel( mail_folder )
    % for each file in the mail folder
    for i_file = 1 : numel( mail_folder{i_folder} )
        % if the file has something in it
        if mail_folder{ i_folder }( i_file ).bytes > 0
            
            %%% select current file
            file_temp = [mail_folder{ i_folder }( i_file ).folder '/' mail_folder{ i_folder }( i_file ).name] ;
            %%%
        ...
        [i_folder / numel(mail_folder) * 100 i_file / numel(mail_folder{i_folder}) * 100]
        
    end
    
end

save([folder_fiu 'mail/webmail_fiu_data.mat'] , ...
    'count_read_bin_30mnt' , ...
    'size_read_bin_30mnt' , ...
    'sl_read_bin_30mnt' , ...
    'burst_read_bin_30mnt' , ...
