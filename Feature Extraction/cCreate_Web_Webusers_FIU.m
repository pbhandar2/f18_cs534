...
web_folder{1} = dir( 'FIU/web/webusers' ) ;
...
% for each web folder
for i_folder = 1 : numel( web_folder )
    % for each file in the web folder
    for i_file = 1 : numel( web_folder{i_folder} )
        % if the file has something in it
        if web_folder{ i_folder }( i_file ).bytes > 0
            
            %%% select current file
            file_temp = [web_folder{ i_folder }( i_file ).folder '/' web_folder{ i_folder }( i_file ).name] ;
            %%%
        ...
        [i_folder / numel(web_folder) * 100 i_file / numel(web_folder{i_folder}) * 100]
        
    end
    
end

save([folder_fiu 'web/webusers_fiu_data.mat'] , ...
    'count_read_bin_30mnt' , ...
    'size_read_bin_30mnt' , ...
    'sl_read_bin_30mnt' , ...
    'burst_read_bin_30mnt' , ...
